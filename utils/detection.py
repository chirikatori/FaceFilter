...
def detect_faces(model,
    ...
    for i, image_result in enumerate(results):
        ...
        if box_format == 'xyxy':
            ...
            image_boxes = image_result.boxes.xyxy.cpu().numpy()
            image_boxes = [[round(x1), round(y1), round(np.abs(x1-x2)),
                            round(np.abs(y1-y2))]
            ...
            image_boxes = image_result.boxes.xyxyn.cpu().numpy()
            image_boxes = [[float(x1), float(y1), float(np.abs(x1-x2)),
                            float(np.abs(y1-y2))]
                           for x1, y1, x2, y2 in image_boxes]
        ...
        for j in range(len(image_boxes)):
            (x1, y1, x2, y2), score, c = image_boxes[j], image_scores[j], image_cls[j]  # noqa
            if score >= th:
                ...
                if box_format == 'xyxy':
                    ...
                    filtered_crops.append(img.crop([x1*img_w, y1*img_h,
                                                    x2*img_w, y2*img_h]))
                    filtered_boxes.append([float(x1), float(y1),
                                           float(x2), float(y2)])
                elif box_format == 'xywh':
